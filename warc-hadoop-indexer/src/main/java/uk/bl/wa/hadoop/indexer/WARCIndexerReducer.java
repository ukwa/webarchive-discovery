...
public class WARCIndexerReducer extends MapReduceBase implements
	Reducer<Text, WritableSolrRecord, Text, Text> {
	...
	try {
		...
		if (conf.hasPath("warc.solr.zookeepers")) {
		    ...
		    solrServer = new CloudSolrServer(
			    conf.getString("warc.solr.zookeepers"));
    ...
    @Override
    public void reduce(Text key, Iterator<WritableSolrRecord> values,
