...
public class WARCIndexerMapper extends MapReduceBase implements
	Mapper<Text, WritableArchiveRecord, Text, WritableSolrRecord> {
	...
	public UriCollection(String collectionCategories,
		String allCollections, String subject) {
	...
	try {
	    ...
	    Config config = ConfigFactory.parseString(job
		    .get(WARCIndexerRunner.CONFIG_PROPERTIES));
	    // If we're reading from ACT, parse the XML output into our
	    // collection lookup.
    ...
    public void map(Text key, WritableArchiveRecord value,
	    OutputCollector<Text, WritableSolrRecord> output, Reporter reporter)
	    throws IOException {
	...
	if (!header.getHeaderFields().isEmpty()) {
	    ...
	    Text oKey = new Text(
		    (String) solr.doc.getFieldValue(SolrFields.SOLR_HOST));
	    try {
    ...
    /**
     * Runs through the 3 possible scopes, determining the appropriate part of
     * the URI to match.
     * 
     * @param uri
     * @param solr
     ...
     * @throws URIException
     */
    private void processCollectionScopes(URI uri, SolrRecord solr)
	    throws URISyntaxException, URIException {
	// "Just this URL".
	if (collections.get("resource").keySet()
		.contains(canon.urlStringToKey(uri.toString()))) {
	    updateCollections(collections.get("resource").get(uri.toString()),
		    solr);
	}
	...
	HashMap<String, UriCollection> subdomains = collections
		.get("subdomains");
	...
	if (collection.collectionCategories != null
		&& collection.collectionCategories.length() > 0) {
	    if (collectionDateRanges
		    .containsKey(collection.collectionCategories)
		    && collectionDateRanges
			    .get(collection.collectionCategories)
			    .isInDateRange(date)) {
		solr.addField(SolrFields.SOLR_COLLECTION,
			collection.collectionCategories);
		LOG.info("Added collection " + collection.collectionCategories
			+ " to " + solr.doc.getField(SolrFields.SOLR_URL));
	    }
	}
	// Iterate over the hierarchical collections
	if (collection.allCollections != null
		&& collection.allCollections.length > 0) {
	    for (String col : collection.allCollections) {
		if (collectionDateRanges.containsKey(col)
			&& collectionDateRanges.get(col).isInDateRange(date)) {
		    solr.addField(SolrFields.SOLR_COLLECTIONS, col);
		    LOG.info("Added collection '" + col + "' to "
			    + solr.doc.getField(SolrFields.SOLR_URL));
		}
	    }
	}
	...
	if (collection.subject != null && collection.subject.length > 0) {
		...
		if (collectionDateRanges.containsKey(subject)
			&& collectionDateRanges.get(subject)
				.isInDateRange(date)) {
		    solr.addField(SolrFields.SOLR_SUBJECT, subject);
		    LOG.info("Added collection '" + subject + "' to "
			    + solr.doc.getField(SolrFields.SOLR_URL));
		}
	    }
	}
    ...
    private void parseCollectionXml(String xml) throws JDOMException,
	    IOException {
	...
	for (int i = 0; i < list.size(); i++) {
		...
		start = node.getChild(FIELD_DATES).getChildText(
			FIELD_START_DATE);
		end = node.getChild(FIELD_DATES).getChildText(FIELD_END_DATE);
		dateRange = new DateRange(start, end);
		LOG.debug("Valid collection: " + name);
     ...
     * @return
     */
    private UriCollection filterUriCollection(String collectionCategories,
	    String allCollections, String subject) {
	UriCollection output = null;
	Set<String> validCollections = collectionDateRanges.keySet();

	if (collectionCategories != null
		&& !validCollections.contains(collectionCategories)) {
	...
	if (collectionCategories != null || allCollections != null
		|| subject != null)
	    output = new UriCollection(collectionCategories, allCollections,
		    subject);
	...
	for (int i = 0; i < list.size(); i++) {
	    ...
	    // Trac #2271: Erroneous data in ACT might contain pipe-separated
	    // text.
	    if (collectionCategories != null
		    && collectionCategories.indexOf("|") != -1) {
	    ...
	    // As long as one of the fields is populated we have something to
	    // do...
	    if (collectionCategories != null || allCollections != null
		    || subject != null) {
		UriCollection collection = filterUriCollection(
			collectionCategories, allCollections, subject);
		// There should be no scope beyond those created in the
		// Constructor.
		if (collection != null)
	...
	for (String key : collections.keySet()) {
	    LOG.info("Processed " + collections.get(key).size()
		    + " URIs for collection " + key);
	}
    }

    private void addCollection(String scope, String urls,
	    UriCollection collection) {
	HashMap<String, UriCollection> relevantCollection = collections
		.get(scope);
