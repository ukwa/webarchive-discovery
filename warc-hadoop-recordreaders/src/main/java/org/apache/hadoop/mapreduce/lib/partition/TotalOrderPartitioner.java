 ...
 * 
 * BACKPORTED HERE AS WE RUN A REALLY OLD HADOOP - ANJ
 * 
 */
public class TotalOrderPartitioner<K extends WritableComparable<?>, V>
    ...
    class BinarySearchNode implements Node<K> {
        ...
        public int findPartition(K key) {
            final int pos = Arrays.binarySearch(splitPoints, key, comparator)
                    + 1;
     ...
     * @param splitPoints
     *            the full split point vector, which holds the split point or
     *            points this leaf node should contain
     * @param lower
     *            first INcluded element of splitPoints
     * @param upper
     *            first EXcluded element of splitPoints
    ...
    private class LeafTrieNode extends TrieNode {
        ...
        LeafTrieNode(int level, BinaryComparable[] splitPoints, int lower,
                int upper) {
            super(level);
            this.lower = lower;
            this.upper = upper;
        ...
        public int findPartition(BinaryComparable key) {
            final int pos = Arrays.binarySearch(splitPoints, lower, upper, key)
                    + 1;
            return (pos < 0) ? -pos : pos;
        }
    }
    ...
    private class SinglySplitTrieNode extends TrieNode {
        ...
        SinglySplitTrieNode(int level, BinaryComparable[] splitPoints,
                int lower) {
            super(level);
            this.lower = lower;
  ...
  /**
     ...
     */
    private TrieNode buildTrie(BinaryComparable[] splits, int lower, int upper,
            byte[] prefix, int maxDepth) {
        return buildTrieRec(splits, lower, upper, prefix, maxDepth,
                new CarriedTrieNodeRef());
    }
    ...
    private TrieNode buildTrieRec(BinaryComparable[] splits, int lower,
        ...
        for (int ch = 0; ch < 0xFF; ++ch) {
            ...
            trial[depth] = (byte) ch;
            result.child[0xFF & ch] = buildTrieRec(splits, lower, currentBound,
                    trial, maxDepth, ref);
        }
        // pick up the rest
        trial[depth] = (byte) 0xFF;
        result.child[0xFF] = buildTrieRec(splits, lower, currentBound, trial,
                maxDepth, ref);
